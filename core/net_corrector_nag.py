...
class NAG:
    ...
    def correct(self, net_object, expected_output_vector):
        ...
        correcting_net = net_object

        # pprint.pprint(net_object.layers)
        ...
        for previous_layer, current_layer in zip(correcting_net.layers, correcting_net.layers[1:-1]):
            for neuron in current_layer[1:]:
                synapse_sum = sum(starmap(lambda x, y: x['d_output'] * y, zip(previous_layer[1:], neuron['synapses'])))
                neuron['d_input'] = synapse_sum
                neuron['d_output'] = self.__df(sigmoid=neuron['output'], **self.__df_param)

        for neuron in correcting_net.layers[-1]:
            synapse_sum = sum(starmap(lambda x, y: x['d_output'] * y, zip(previous_layer[1:], neuron['synapses'])))
            neuron['d_input'] = synapse_sum
            neuron['d_output'] = self.__df(sigmoid=neuron['output'], **self.__df_param)
        ...
        for layer_n in range(len(correcting_net.layers) - 2, -1, -1):
            for neuron_n in range(1, len(correcting_net.layers[layer_n])):
                sigma = sum([n['sigma'] * n['synapses'][neuron_n] for n in correcting_net.layers[layer_n + 1][1:]])
        ...
        for layer_n in range(1, len(correcting_net.layers)):
            for neuron_n in range(1, len(correcting_net.layers[layer_n])):
                for synapse_n in range(len(correcting_net.layers[layer_n][neuron_n]['synapses'])):
                    ...
                    delta = -self.__nu * (1 - self.__mu) * sigma * output
